...
static void app_repaint(struct window_t* window, void* userdata) {
    ...
    grille_view_repaint(window, app->view);
}

/** La couleur blanche (solide) */
static const SDL_Color WHITE = { .r = 0xFF, .g = 0xFF, .b = 0xFF, .a = 0xFF };

struct app_t* setup_app(int width, int height, grille_t* grille) {
    ...
    app->window = create_window(width, height, "Dijkstra - Recherche de chemin", app_repaint, app);
    app->view = create_grille_view_vp(app->window, grille, area);
    get_viewport(app->view, &(app->viewport));

    set_background_color(app->window, WHITE);

    app->thread = NULL;

    // On setup les animations
    setup_animation(app, app->view);

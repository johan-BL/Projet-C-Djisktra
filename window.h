...
/**
 * Rend un rectangle (vide) sur la fenêtre, dessinantn que les côtés.
 *
 ...
 * @param color couleur du rectangle (pour la bordure)
 */
 ...
 */
void render_rect_fill(struct window_t* window, const SDL_Rect* rect, SDL_Color color);
...
/**
 * Rend un disque (plein) sur la fenêtre
 *
 * Pré-conditions : window non NULL, window lancée, thread = render thread de la fenêtre
 * r > 0
 *
 ...
 * @param color couleur du cercle
 */
void render_circle_fill(struct window_t* window, int x, int y, int r, SDL_Color color);
